...
export function RhythmGrid(props: RhythmGridProps) {
  ...
  return <div className="editor-grid" ref={props.gridRef} onWheel={horizontalWheelScroll}><div className="editor-horizontal" style={{ width: `${160 + totalBars * 960}px` }}>
    ...
    <div className="editor-measure-line"><span className="editor-corner">PISTES</span><div className="editor-measure-heads" style={{ gridTemplateColumns: `repeat(${totalBars}, 1fr)` }}>{Array.from({ length: totalBars }, (_, measure) => {
      ...
      return <b className={`${committed ? 'committed' : ''} ${committed?.localMeasure === 0 ? 'section-start' : ''} ${committed && committed.localMeasure === committed.section.bars - 1 ? 'section-end' : ''} ${!committed && draftMeasure === props.bars - 1 ? 'reserve' : ''}`} key={measure}>{committed ? `${committed.section.label} · MESURE ${committed.localMeasure + 1}` : `MESURE ${draftMeasure + 1}${draftMeasure === props.bars - 1 ? ' · SUITE' : ''}`}</b>;
    })}</div></div>
    <div className="editor-step-line"><span className="editor-corner">PAS</span><div style={{ gridTemplateColumns: `repeat(${totalBars * 16}, 1fr)` }}>{Array.from({ length: totalBars * 16 }, (_, globalStep) => <b className={`measure-tone-${Math.floor(globalStep / 16) % 2} ${globalStep < committedBars * 16 ? 'committed' : ''}`} key={globalStep}>{globalStep % 16 + 1}</b>)}</div></div>
    {EP133_SCORE_TRACKS.map((track) => {
      ...
      return <div className={`editor-horizontal-row ${props.mode === 'complete' && props.selectedPad === track.pad ? 'selected-pad' : ''} ${melodic ? 'melodic-track' : ''}`} key={track.pad}><strong onClick={() => { props.onSelectPad(track.pad); if (melodic) props.onOpenKeys(); }}>{props.padName(track.pad)} · {props.group}-{EP133_PADS[track.pad].key}{props.mode === 'complete' ? ` · ${melodic ? 'KEYS ♫' : 'ONE'}` : ''}</strong><div style={{ gridTemplateColumns: `repeat(${totalBars * 16}, 1fr)` }}>{Array.from({ length: totalBars * 16 }, (_, globalStep) => {
        ...
        return <button className={`${stepTargets.length ? 'checked' : ''} ${committed ? 'committed' : ''} ${committed?.localMeasure === 0 && step === 0 ? 'section-start' : ''} ${committed && committed.localMeasure === committed.section.bars - 1 && step === 15 ? 'section-end' : ''} ${!committed && localMeasure === props.bars - 1 ? 'reserve' : ''}`} onClick={() => committed ? props.onToggleCommittedStep?.(committed.section.key, localMeasure, track.pad, step) : melodic ? (props.onSelectPad(track.pad), props.onOpenKeys()) : props.onToggleStep(localMeasure, track.pad, step)} aria-label={`${props.padName(track.pad)}, mesure ${measure + 1}, pas ${step + 1}`} key={globalStep}>{stepTargets.length ? noteLabel || EP133_PADS[track.pad].key : ''}</button>;
      })}</div></div>;
    })}
  </div></div>;
}
