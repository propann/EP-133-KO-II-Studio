...
export function MachineCloneDialog({ inventory, soundIndex, onClose }: MachineCloneDialogProps) {
  ...
  const statusManifest = cloneStatus?.manifest as { status?: string; progress?: { phase?: string; current?: number; total?: number; elapsedSeconds?: number; estimatedRemainingSeconds?: number }; summary?: { errorCount?: number } } | undefined;
  ...
  return <div className="machine-clone-overlay" role="dialog" aria-modal="true" aria-labelledby="clone-title"><section className="machine-clone-dialog">
    ...
    <p><b>PRÉVOIR 20 À 30 MINUTES POUR LE PREMIER CLONE COMPLET.</b> Mesure réelle : 25 min 20 s pour 9 projets et 527 samples. La progression indiquera phase, compteur, temps écoulé et estimation restante. Cette opération n'écrit rien sur l'EP‑133.</p>
