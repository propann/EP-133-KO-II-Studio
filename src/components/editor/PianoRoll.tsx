...
export function PianoRoll({ gridRef, group, selectedPad, bars, playing, playbackBeat, targets, onClose, onPreviewNote, onToggleNote }: PianoRollProps) {
  return <div className="key-editor"><div className="key-editor-title"><span>GROUPE {group} · PAD {EP133_PADS[selectedPad].key} · {EP133_PADS[selectedPad].name}</span><b>MODE KEYS · PIANO-ROLL</b><button onClick={onClose}>RETOUR AUX 12 PADS</button></div><div className="editor-grid key-grid" ref={gridRef} onWheel={horizontalWheelScroll}><div className="key-roll" style={{ width: `max(100%, ${160 + bars * 960}px)` }}>
    {playing && <i className="editor-playhead" style={{ left: `${160 + playbackBeat / 4 * 960}px` }} />}
    <div className="editor-measure-line"><span className="editor-corner">CLAVIER</span><div className="editor-measure-heads" style={{ gridTemplateColumns: `repeat(${bars}, 1fr)` }}>{Array.from({ length: bars }, (_, measure) => <b className={targets.some((note) => Math.floor(note.beat / 4) === measure) ? 'has-notes' : ''} key={measure}>{measure + 1}/{bars}</b>)}</div></div>
