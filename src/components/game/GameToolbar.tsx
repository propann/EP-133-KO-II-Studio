...
export function GameToolbar(props: GameToolbarProps) {
  ...
  return <header className="toolbar">
    ...
    <label className="mode-select">STYLE <select value={props.styleId} disabled={locked} onChange={(event) => props.onStyleChange(event.target.value)}><optgroup label="STYLES">{props.styles.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</optgroup>{props.userExercises.length > 0 && <optgroup label="USER">{props.userExercises.map((item) => <option value={`user:${item.id}`} key={item.id}>{item.title}</option>)}</optgroup>}</select></label>
