...
const copy = {
  fr: {
    ...
    cards: [['CRÉER','PATTERN & SONG STUDIO','Ouvre des morceaux de la machine et travaille groupes, patterns, scènes et positions Song dans un véritable éditeur.'],['CLONER · GÉRER','SONS & TRANSFERT','Clone projets et samples, inspecte la bibliothèque réelle et prépare précisément les emplacements de la machine.'],['ORDI','BANQUE DE SONS — ORDI','Parcours ta bibliothèque de sons perso, écoute-la et prépare-la pour la machine — glisse-dépose sur les pads.'],['CONNECTER','TEST MACHINE','Inspecte MIDI et SysEx, associe les contrôles physiques et vérifie la communication dans les deux sens.'],['IDENTITÉ','FICHE PERSONNAGE','Avatar, machines déclarées et bilan cumulé de toutes tes sessions de jeu.'],['MODULE INCLUS','RHYTHM HERO','Retrouve les exercices progressifs, partitions animées et entraînements avec les pads de l’EP‑133.'],['COMPRENDRE','DOCUMENTATION','Guides du Studio et de la machine, connexion MIDI, formats et procédures vérifiées.']], official: 'GUIDE OFFICIEL EP‑133 ↗', local: 'OPEN SOURCE · LOCAL · SANS COMPTE', language: 'Langue',
  },
  en: {
  ...
  es: {
    ...
    cards: [['CREAR','PATTERN & SONG STUDIO','Abre música de la máquina y edita grupos, patterns, escenas y posiciones Song.'],['CLONAR · GESTIONAR','SONIDOS Y TRANSFERENCIA','Clona proyectos y samples, inspecciona la biblioteca real y prepara los slots de la máquina.'],['ORDENADOR','BANCO DE SONIDOS — ORDENADOR','Explora tu biblioteca de sonidos, escúchala y prepárala para la máquina — arrastra sobre los pads.'],['CONECTAR','PRUEBA DE MÁQUINA','Inspecciona MIDI y SysEx, asigna controles físicos y verifica la comunicación bidireccional.'],['IDENTIDAD','FICHA DE PERSONAJE','Avatar, máquinas declaradas y balance acumulado de todas tus sesiones de juego.'],['MÓDULO INCLUIDO','RHYTHM HERO','Practica con ejercicios progresivos, partitura animada y los pads reales del EP‑133.'],['COMPRENDER','DOCUMENTACIÓN','Guías del Studio y de la máquina, conexión MIDI, formatos y procedimientos verificados.']], official: 'GUÍA OFICIAL EP‑133 ↗', local: 'CÓDIGO ABIERTO · LOCAL · SIN CUENTA', language: 'Idioma',
  },
} as const;
...
export function HomePage({ connected, project, scannedSoundCount, language, onLanguageChange, onOpenGame, onOpenStudio, onOpenSounds, onOpenDocumentation, onOpenMachineTest, onOpenProfile, onOpenLocalSounds }: HomePageProps) {
  const t = copy[language];
  const actions = [onOpenStudio, onOpenSounds, onOpenLocalSounds, onOpenMachineTest, onOpenProfile, onOpenGame, onOpenDocumentation];
  const classes = ['studio-card', 'sounds-card', 'local-sounds-card', 'machine-test-card', 'profile-card', 'game-card', 'docs-card'];
